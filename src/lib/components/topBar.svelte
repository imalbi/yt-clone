...
<nav class="bg-primary text-primary mt-2 flex w-full items-center justify-between">
	{#if showOnlySearch}
		<div class="flex w-full items-center">
			<button
				class="border-border-color hover:bg-secondary h-10 cursor-pointer rounded-l-full border border-r-0 px-4"
				aria-label="Chiudi"
			...
			<form class="flex-1" {onsubmit}>
				<input
					...
					class="border-border-color bg-secondary text-primary h-10 w-full rounded-r-full border px-4"
					placeholder="Cerca"
				/>
			</form>
		</div>
	{:else}
		...
		<div class="order-2 flex flex-shrink-0 items-center">
			<form class="flex" {onsubmit}>
				<input
					bind:value={inputSearch}
					class="border-border-color bg-secondary text-primary hidden h-10 w-[25vw] rounded-l-3xl border md:block"
					type="text"
					placeholder="Cerca"
				/>
				<button
					type="submit"
					class="border-border-color bg-secondary hover:bg-secondary h-10 cursor-pointer rounded-full border p-2 md:rounded-l-none md:rounded-r-full"
		...
		<div class="order-3 mr-1 flex flex-shrink-0 items-center gap-1">
			<button class="hover:bg-secondary flex h-10 cursor-pointer items-center rounded-3xl"
				><img class="h-9" src={add} alt="" />Crea</button
			>
			<button class="hover:bg-secondary h-10 w-10 cursor-pointer rounded-full"
