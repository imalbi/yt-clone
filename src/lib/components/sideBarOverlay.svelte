...
{#if isOpen}
	...
	<aside
		class="bg-background fixed top-0 bottom-0 left-0 z-50 flex w-60 flex-col pt-2 pr-10"
		in:fly={{ x: -256, duration: 300, easing: quintOut }}
		out:fly={{ x: -256, duration: 250, easing: quintOut }}
	>
		<div class="text-primary ml-1 flex items-center gap-3 md:whitespace-nowrap">
			<button
				class="hover:bg-background-secondary flex h-12 w-12 flex-shrink-0 cursor-pointer items-center justify-center rounded-full p-2.5"
				onclick={close}
		...
		<nav class="fixed top-20 left-3 flex flex-col gap-2">
			{#each items as item}
				<a
					href={item.ref}
					class="hover:bg-background-secondary flex w-50 cursor-pointer items-center gap-4 rounded-xl p-2 py-4"
				>
					<span class="text-primary h-7 w-7 flex-shrink-0">
